# Novel-to-Script Pro v2.2

> 🎬 小说 → 剧本 → 分镜 → 生图，AI 驱动的全流程智能改编系统

**Novel-to-Script Pro** 是一款 AI 辅助剧本创作工具。上传小说 TXT 章节或**在线爬取**，自动生成结构化 YAML 剧本、审核报告、分镜序列板和 AI 图片。

v2.1 独创**剧情权重分级算法**与**影视节奏智能拆分**，v2.2 新增**在线小说爬取**、**剧本微调**和**动态前端动效**，打破市面工具"无脑全文直译"的弊端，实现真正专业的影视改编。
...
3. 「导入」标签页上传 `.txt` 章节文件
4. 点「运行管线」
5. 查看「剧本」(含S/A/B分级统计 + **微调**)、「规划」(冲突节点+剧集标注)、「审核」「分镜」「图片」
